...
function user_edit(id) {
    $("input.select-user-group").uCheck('uncheck');
    $.ajax({
        type: "POST",
        ...
        success: function (response) {
            ...
            $('#edit-seatnumber').val(response.number);
            response.groups.forEach(ele => {
                $(`input.select-user-group[value='${ele.id}']`).uCheck('check');
            });
            $('#edit-modal').modal({
                relatedTarget: this,
                onConfirm: function (e) {
                    //get user groups
                    grps = [];
                    $("input.select-user-group:checked").each(function() {
                        grps.push($(this).attr("value"));
                    });
                    grp_text = grps.join();
                    $.ajax({
                        type: "POST",
                        ...
                        data: {
                            ...
                            'number': $('#edit-seatnumber').val().trim(),
                            'groups': grp_text
                        },
                        dataType: "json",
                        success: function (response) {
