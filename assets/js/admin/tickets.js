        ...
        onConfirm: function (e) {
            $.ajax({
                ...
                data: {
                    ...
                    'sms': $('#add-sms').is(':checked') ? 1 : 0,
                    'force': $('#add-force').is(':checked') ? 1 : 0
                },
                dataType: "json",
                success: function (response) {
                    alert(response.msg);
                    $('#add-sms').uCheck('check');
                    $('#add-force').uCheck('uncheck');
                },
                error: function () {
...
function inputUserInfo(id) {
    $.ajax({
        type: "POST",
        url: "/admin/api_user_info_by_email",
        data: {
            'email': id
        },
        dataType: "json",
        success: function (response) {
            ...
            $('#add-phone').val(response.phone);
        }
    });
}

$("#add-email").blur(function () {
    id = $("#add-email").val().trim();
    if (id) inputUserInfo(id);
});